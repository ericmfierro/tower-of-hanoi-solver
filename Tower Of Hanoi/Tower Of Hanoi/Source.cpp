...
void moveTopDiskFromSourceToDestinationPole(int poleArrayPar[][COLSIZE], string
	sourcePar, string destPar);
...
void displayArray(int poleArrayPar[][COLSIZE]) {
	...
	for (row = ROWSIZE - 1; row >= 0; row--) {
		for (col = 0; col < COLSIZE; col++) {
			if (!(poleArrayPar[row][COL0] == 0 && poleArrayPar[row][COL1] ==
				0 && poleArrayPar[row][COL2] == 0)) {
		...
		if (!(poleArrayPar[row][COL0] == 0 && poleArrayPar[row][COL1] == 0 &&
			poleArrayPar[row][COL2] == 0)) {
			cout << endl;
		}
	}
	cout << "-----" << endl;
